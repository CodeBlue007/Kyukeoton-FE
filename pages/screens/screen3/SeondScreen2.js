...
const SecondScreen2 = ({ navigation }) => {
    ...
    useEffect(() => {
        axios.get(`http://13.124.233.9:8080/categories/5/questions?page=1`)
            .then(({ data }) => {
                setfullItems(data);
                setAnswerList(data.answers);
                setLoading(false);
    ...
    return (
        <LinearGradient colors={['#F39912', '#FFCC00', '#FFCC00']} style={SecondScreenStyle.container}>
            ...
            <ProgressBar curNum={2} backGroundProp={'#F39C12'} />
            <Modall isModalVisible={isModalVisible} setIsModalVisible={setIsModalVisible} correct={correct} next={"SecondScreen3"} navigation={navigation} solution={solution} />
        </LinearGradient >
    )
}
